...
export default function plugin({ types: t }: any,
  ...
  const caller: { callee: string, name: string }[] = []
  ...
  return {
    visitor: {
      ...
      CallExpression(path: any, { filename }: any) {
        ...
        if(t.isIdentifier(callee)) {
           ...
           * ```
           */
          replaceFilter('Identifier', (callee: any) => callee.name)
           ...
           * ```
           */
          replaceFilter('MemberExpression', (callee: any) => callee.object.name)

          return
        }

        function replaceFilter(target: string, getName: Function) {
